# Working on Wiz documentation

Read `WIZ.md` for the product model before changing public behavior.

## Ownership

This repository owns the Astro Starlight site. Examples and diagnostics must match released behavior, links must target the `wiz-sh` repositories, and every documentation build must remain warning-free where practical.
